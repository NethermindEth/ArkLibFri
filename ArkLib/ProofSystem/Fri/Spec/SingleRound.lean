...
noncomputable def foldProver :
  OracleProver (oSpec D x) (Statement F i.castSucc) (OracleStatement D x i.castSucc) (Witness F)
        ...
        (List.finRange k).mapM
              (fun (i : Fin k) =>
                do
                  ...
                  if h : i.1 < k - 1
                  then
                    let β  ← sampleCodeword (k := k) D x ⟨i.1.succ, by omega⟩
